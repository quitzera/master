$(function () {
  ...
  $(document).on("pageInit", "#page-infinite-scroll-bottom", function(e, id, page) {
    ...
    // 最多可加载的条目
    var maxItems = 100;
    var lastIndex = $('#ulGoodsList>li').length;
    // console.log($('#ulGoodsList>li'));
    function addItems(number, lastIndex) {
      // 生成新条目的HTML
      var html = '';
      for (var i = lastIndex + 1; i <= lastIndex + number; i++) {
        html += '<li id="23468">';    
        html += '<span class="gList_l fl">';         
                       html += '<img src="https://img.1yyg.net/GoodsPic/pic-200-200/20160908104402359.jpg">'; 
                   html += '</span>';    
                   html += '<div class="gList_r">';         
                       html += '<h3 class="gray6">(第17094云)苹果（Apple）iPhone 7 Plus 256G版 4G手机</h3>';         
                       html += '<em class="gray9">价值：￥7988.00</em>'; 
                       html += '<div class="gRate">';           
                           html += '<div class="Progress-bar">';     
                               html += '<p class="u-progress">'; 
                                   html += '<span style="width: 91.91286930395593%;" class="pgbar">'; 
                                       html += '<span class="pging"></span>'; 
                                   html += '</span>'; 
                               html += '</p>';                 
                               html += '<ul class="Pro-bar-li">'; 
                                   html += '<li class="P-bar01"><em>7342</em>已参与</li>'; 
                                   html += '<li class="P-bar02"><em>7988</em>总需人次</li>'; 
                                   html += '<li class="P-bar03"><em>646</em>剩余</li>'; 
                               html += '</ul>';             
                           html += '</div>';            
                           html += '<a codeid="12785750" class="" canbuy="646"><s></s></a>';         
                       html += '</div>';     
                   html += '</div>'; 
               html += '</li>';
      }
      // 添加新条目
      console.log('a');
      $('.goodList #ulGoodsList').append(html);
    }
  ...
  //   });
  //   //点击时打开图片浏览器
  //   $(page).on('click','.slides li',function () {
